...
2. Run fresh research for the paid-product desire lane, current FREE-product
   lane, and honest transaction-bridge lane.
3. Use `$freeperrylarp1-brainstorm` once at the angle decision point.
4. Accept one primary and one backup angle in a marriage brief.
5. Validate that brief before writing.
6. Write only from the accepted brief and current evidence.
7. Validate copy, image-job visible text, prompt directions, and site data
   against the public boundary and prior-product registry.
8. Continue to prompt delivery only after every gate passes.
